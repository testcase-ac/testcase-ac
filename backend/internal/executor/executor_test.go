...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := util.CleanStdout(runResult.Stdout, "no"); got != "OJ BOJ" {
...
`
	...
	if got := util.CleanStdout(runResult.Stdout, "no"); got != "3" {
...
func TestRunCodeSimpleAPlusBSupportedLanguages(t *testing.T) {
	for _, tc := range supportedLanguageSmokeCases() {
		t.Run(tc.Name, func(t *testing.T) {
			...
			if got := util.CleanStdout(runResult.Stdout, "no"); got != tc.Want {
