...
`outputOnly: true`인 문제를 제외하고 `generator_*`, `singlegen_*`, 또는 고정
테스트케이스 중 하나 이상도 필요합니다.
...
- `outputOnly`: 입력이 없는 문제에만 `true`로 설정합니다. 이런 문제는 빈
  표준 입력 케이스 하나를 암묵적으로 사용하므로 벨리데이터, 제너레이터, 고정
  테스트케이스, 답안 파일을 추가하지 않습니다.
...
### 거부되어야 하는 코드 (`rejected_*`) (선택)

검증 전용으로 보관하는 거부되어야 하는 코드입니다. 확장자를 제외한 파일 이름이
`rejected`이거나 `rejected_`로 시작해야 합니다. 예: `rejected_slow.cpp`,
`rejected.py`. 이 코드는 UI나 API에 표시되지 않습니다. 반드시 컴파일되어야 하며,
전체 저장소 검증에서 각 파일은 현재 테스트케이스 공급자 또는 출력 전용 문제의 빈
표준 입력 케이스 중 하나 이상에서 비정답 판정을 받아야 합니다.

### 제너레이터 코드 (`generator_*`) (선택)

랜덤 테스트 제너레이터입니다. 파일 이름이 `generator`로
시작하면 제너레이터로 읽힙니다. 예: `generator_random.cpp`. 실행 파일은
첫 번째 명령줄 인자로 시드를 받아야 하며, 같은 시드에 대해 동일한 출력을 만들어야 합니다. (testlib.h를 사용한 제너레이터는 자동으로 첫 번째 명령줄 인자로 시드를 받습니다.)

### 단일 제너레이터 코드 (`singlegen_*`) (선택)

고정된 출력을 만드는 제너레이터입니다. 출력이 너무 커서
텍스트로 커밋하기 어려운 경우 사용합니다. 파일 이름이
...
고정 테스트케이스는 2 MiB(2097152 바이트) 이하여야 합니다. 더 큰 입력은
...
검증 과정에서 정답 코드의 출력이 답안 파일과 일치하는지 확인합니다. `checker.cpp`가
있으면 체커로 비교하고, 없으면 기본 출력 비교를 사용합니다.

### `validator.cpp` (출력 전용 문제가 아니면 필수)
