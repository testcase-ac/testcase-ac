...
def fi(C, path, i, m):
    new_path = [[path[j][0], path[j][1]+dy(path, m, i, j)] for j in range(max(0, i-W), min(i+W+1, len(path)))]
    return F(C, new_path)

def move(path, i, m):
    for j in range(max(0, i-W), min(i+W+1, len(path))):
...
def radioactive_islands():
    N, A, B = map(float, input().strip().split())
    ...
    for i in range(len(regions)-1):
        ...
        path.extend([X_START+j*H, mid_y + 1.0*(j-mid)/(PATH_LEN-1-mid)*(B-mid_y)]
                    for j in range(mid, PATH_LEN))
        result = min(result, hill_climbing(C, path))
    return result

IT = 1600  # tuned by experiment
W = 2  # interval window size is 2W+1 for move of hill-climbing, ex. move (0, 1, 2, 1, 0), tuned by experiment
