...
int main() {
  ...
  for (int i = 5; i <= n; i++) {
    ...
    for (auto knum : knums) {
      if (knum > i) break;
      if (dp[i - knum].first != -1 && mins > dp[i - knum].second) {
        mins = dp[i - knum].second;
        dp[i] = {knum, mins + 1};
      };
    }
  }
