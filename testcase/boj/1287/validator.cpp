...
// Stage 1: syntax-only parsing (no evaluation)
bool parseExprCheck(int &pos);

bool parseNumberCheck(int &pos) {
    if (pos >= n || !isdigit(expr[pos])) return false;
    while (pos < n && isdigit(expr[pos])) pos++;
    return true;
}

bool parseFactorCheck(int &pos) {
    if (pos >= n) return false;
    if (isdigit(expr[pos])) {
        return parseNumberCheck(pos);
    } else if (expr[pos] == '(') {
        pos++; // '('
        if (!parseExprCheck(pos)) return false;
        if (pos >= n || expr[pos] != ')') return false;
        pos++; // ')'
        return true;
    } else {
        return false;
...
bool parseTermCheck(int &pos) {
    if (!parseFactorCheck(pos)) return false;
...
bool parseExprCheck(int &pos) {
    if (!parseTermCheck(pos)) return false;
    while (pos < n && (expr[pos] == '+' || expr[pos] == '-')) {
        pos++; // operator
        if (!parseTermCheck(pos)) return false;
    }
...
// Stage 2: evaluation with BigInt and division property check
bool divZeroEncountered;

bool parseExprEval(int &pos, BigInt &res);

bool parseNumberEval(int &pos, BigInt &res) {
    if (pos >= n || !isdigit(expr[pos])) return false;
    int start = pos;
    while (pos < n && isdigit(expr[pos])) pos++;
    string s = expr.substr(start, pos - start);
    res = BigInt(s);
    return true;
}

bool parseFactorEval(int &pos, BigInt &res) {
    if (pos >= n) return false;
    if (isdigit(expr[pos])) {
        return parseNumberEval(pos, res);
    } else if (expr[pos] == '(') {
        pos++; // '('
        if (!parseExprEval(pos, res)) return false;
        if (pos >= n || expr[pos] != ')') return false;
        pos++; // ')'
        return true;
    } else {
...
bool parseTermEval(int &pos, BigInt &res) {
    BigInt cur;
    if (!parseFactorEval(pos, cur)) return false;
    while (pos < n && (expr[pos] == '*' || expr[pos] == '/')) {
        char op = expr[pos++];
        BigInt rhs;
        if (!parseFactorEval(pos, rhs)) return false;

        if (op == '*') {
            if (!divZeroEncountered) {
                cur = cur * rhs;
            }
            // if divZeroEncountered, we can skip heavy computation; cur not needed anymore
        } else { // '/'
            if (rhs == 0) {
                divZeroEncountered = true;
                cur = BigInt(0); // keep values small afterwards
            } else {
                if (!divZeroEncountered) {
                    BigInt r = cur % rhs;
                    ensuref(r == 0,
                            "Non-integer division detected at operator position %d (0-based)",
                            pos - 1);
                    cur = cur / rhs;
                } else {
                    // already invalid due to earlier division by zero; skip
                }
            }
        }
    }
...
bool parseExprEval(int &pos, BigInt &res) {
    BigInt cur;
    if (!parseTermEval(pos, cur)) return false;
    while (pos < n && (expr[pos] == '+' || expr[pos] == '-')) {
        char op = expr[pos++];
        BigInt rhs;
        if (!parseTermEval(pos, rhs)) return false;
        if (op == '+') cur = cur + rhs;
        else cur = cur - rhs;
    }
    res = cur;
...
int main(int argc, char* argv[]) {
    ...
    expr = inf.readToken("[^]+", "expr");
    inf.readEoln();

    n = (int)expr.size();
    ensuref(1 <= n && n <= 1000, "Expression length must be between 1 and 1000, got %d", n);
    ...
    int pos1 = 0;
    bool syntaxOk = parseExprCheck(pos1) && (pos1 == n);

    // Stage 2: if syntactically valid, enforce global division property
    if (syntaxOk) {
        int pos2 = 0;
        divZeroEncountered = false;
        BigInt value;
        bool evalOk = parseExprEval(pos2, value) && (pos2 == n);
        ensuref(evalOk, "Internal error: evaluation parse disagrees with syntax parse");
        // If evalOk, non-integer divisions have already been rejected via ensuref
        // Division by zero is allowed (expression is then invalid for the solution).
    }
