...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i <= m; ++i) {
        ...
        (void)w;
        // CHECK: The statement forbids u == v but does not forbid multiple roads between
        // the same cities, so parallel edges are accepted.
    }
    ...
    for (int i = 1; i <= p; ++i) {
        ...
        ensuref(seen.insert(y).second, "duplicate intermediate vertex at index %d: %d", i, y);
    }
