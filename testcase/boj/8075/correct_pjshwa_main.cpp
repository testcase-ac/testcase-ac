...
const int MAXK = 26, MAXN = 1e4;
int A[MAXK + 1];
vector<pii> adj[MAXK + 1][MAXN + 1];
bool vis[MAXK + 1][MAXN + 1];

void dfs(int k, int n) {
  vis[k][n] = true;
  for (auto [nk, nn] : adj[k][n]) if (!vis[nk][nn]) dfs(nk, nn);
}
...
void solve() {
  ...
  if (tot1 != tot2) return cout << "0\n", void();

  int N = tot1;
  adj[0][0].clear(); adj[0][1].clear();
  for (int i = 1; i <= K; ++i) for (int j = 0; j <= N; ++j) adj[i][j].clear();
  memset(vis, 0, sizeof(vis));

  int si = 0, sc = 0, ti = 0, tc = 0;
  for (int j = 1; j <= N; ++j) {
    int sk, sn, tk, tn;
    if (S[si] == '0' || S[si] == '1') sk = 0, sn = S[si] - '0';
    else sk = S[si] - 'a' + 1, sn = sc;
    if (T[ti] == '0' || T[ti] == '1') tk = 0, tn = T[ti] - '0';
    else tk = T[ti] - 'a' + 1, tn = tc;

    adj[sk][sn].emplace_back(tk, tn);
    adj[tk][tn].emplace_back(sk, sn);

    ++sc; ++tc;
    if (S[si] == '0' || S[si] == '1' || A[S[si] - 'a' + 1] == sc) ++si, sc = 0;
    if (T[ti] == '0' || T[ti] == '1' || A[T[ti] - 'a' + 1] == tc) ++ti, tc = 0;
  }

  dfs(0, 0);
  if (vis[0][1]) return cout << "0\n", void();

  dfs(0, 1); InfInt ans = 1;
  for (int k = 1; k <= K; ++k) for (int n = 0; n < A[k]; ++n) {
    if (vis[k][n]) continue;
    dfs(k, n); ans *= 2;
  }
  cout << ans << '\n';
