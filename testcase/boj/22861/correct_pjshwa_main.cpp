...
void solve() {
  ...
  int K; cin >> K;
  while (K--) {
    string path_s, path_d; cin >> path_s >> path_d;
    int fol_si = fidx(split(path_s, "/").back());
    int fol_di = fidx(split(path_d, "/").back());

    // Move files
    for (auto& f : files[fol_si]) files[fol_di].insert(f);
    files[fol_si].clear();

    // Move folders
    for (auto& f : adj[fol_si]) adj[fol_di].push_back(f);
    adj[fol_si].clear();
  }
