...
num ncr(ll n, ll k){
	if(n < k) return 0;
	if(n == 0) return 1;
	return ncr_small(n % P, k % P) * ncr(n / P, k / P);
}

void solve(){
	auto get_coeff = [&](ll c, ll k) -> num {
		num res = 0;
		for(ll d = 0; ; d++){
			ll sgn = (d & 1) ? -1 : 1;
			bool done = true;
			for(ll v : {d * (3*d+1) / 2, d * (3*d-1) / 2}){
				if(v <= k){
					done = false;
					num w = (c == 0) ? num(int(k == v)) : ncr(k-v+c-1, c-1);
					res += sgn * w;
				}
				if(d == 0) break;
			}
	...
	cin >> n >> k;
	assert(k <= 2 * n);
	num ans = get_coeff(n, k);
	if(k >= n+1) ans += get_coeff(n+1, k-n-1);
	cout << ans << '\n';
}
